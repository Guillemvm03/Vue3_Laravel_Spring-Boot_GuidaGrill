  ...
  actions: {
    ...
    [Constant.INITIALIZE_ONE_RESERVATION]: async (store, payload) => {
      try {
        ...
        if (index === -1) {
          const response = await ReservationServiceDashboard.GetReservation(
            payload
          );
          store.commit(Constant.INITIALIZE_ONE_RESERVATION, response.data);
        } else {
          store.commit(
            Constant.INITIALIZE_ONE_RESERVATION,
            store.state.reservations[index]
          );
        }
      } catch (error) {
        console.error(error);
    ...
    [Constant.UPDATE_RESERVATION]: async (store, payload) => {
      try {
        const response = await ReservationServiceDashboard.UpdateReservation(
          payload
        );
    ...
    [Constant.DELETE_RESERVATION]: async (store, payload) => {
      try {
        const response = await ReservationServiceDashboard.DeleteReservation(
          payload
        );
